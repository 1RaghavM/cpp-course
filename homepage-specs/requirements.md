...
- Left: wordmark **cpproad** (monospace or display weight 600). Right: **Sign in** (shadcn `Button variant="ghost"` → `/login`) and **Start learning** (shadcn `Button variant="default"` primary white → `/register`).
...
- Mobile: collapse to a button → shadcn **`Sheet`** menu (full-screen or side drawer). Must be keyboard-operable, focus-trapped, `Esc` to close.
...
- **CTAs:** shadcn `Button variant="default"` **"Start learning C++"** → `/register`; shadcn `Button variant="ghost"` **"Sign in"** → `/login`. Side by side desktop, stacked mobile.
...
- 3–5 shadcn **`Card`** components (`design.md` → Feature card), each anchored to a real C++ pain point, headline + one line + a tiny mono label (shadcn `Badge variant="outline"` with `font-mono`) or 2–3 line code fragment. Concrete topics, e.g.: *Pointers & memory*, *The STL in practice*, *Templates & generics*, *Compile, link, debug*. Each frames the problem then the payoff ("Pointers stop being scary once you can see what they point at").
...
- Show the learning path as an ordered journey. Two acceptable patterns (pick one):
  - **Tabbed modules:** shadcn **`Tabs`** component with underline-style variant (`design.md` → Tabs) grouping stages — *Basics → Memory → STL → Projects* — each `TabsContent` panel listing that stage's topics, optionally a small code sample.
...
- One short line (*"Start with the basics. No setup required."* — placeholder) + **one** shadcn `Button variant="default"` primary white → `/register`. No secondary action here; this is the catch-all conversion point.
