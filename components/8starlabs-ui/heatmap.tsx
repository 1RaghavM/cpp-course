...
const ValueIndicator = forwardRef<HTMLDivElement, ValueIndicatorProps>(
  function ValueIndicator(
    { cellSize, displayStyle, value, maxValue, color, style, ...htmlProps },
    ref
  ) {
    ...
    return (
      <div
        ref={ref}
        className="rounded-md transition-all duration-150 hover:scale-125 hover:brightness-125 hover:z-10"
        style={{
          borderRadius: 4,
          backgroundColor: color,
          ...style
      ...
      <TooltipProvider>
        {weeks.map((week, weekIdx) =>
          week.map((day, dayIdx) => {
            ...
            return (
              <Tooltip key={dayIdx}>
                <TooltipTrigger render={<ValueIndicator style={{ gridColumn: weekIdx + 2, gridRow: dayIdx + 2 }} tabIndex={0} aria-label={`${day}: ${safeValue} event${safeValue !== 1 ? "s" : ""}`} id={`heatmap-cell-${day}`} cellSize={cellSize} displayStyle={displayStyle} value={safeValue} maxValue={maxValue} color={thisColor} />}></TooltipTrigger>
                <TooltipContent>
